namespace ProjectName
    ...
    type public CommandList =
        ...
        static member Help(client: BasicGuildedClient, messageCreated: MessageCreatedEvent, command: string, arguments: IList<string>) =
            ...
            if arguments.Count = 0 then
                ...
                let embed = Embed().SetTitle("Command list").SetDescription(com |> String.concat ", ").AddField("For more info", "Type `help command_name` to get more info about a command.")
                // Output the embed
                embed
                    |> Message.Generate
                    |> messageCreated.RespondAsync
            ...
            // If argument was given
                ...
                // Find that command
                let com = keys.FirstOrDefault(fun x -> x.Name = commandName || x.Alias.Contains(commandName))
                ...
                // If it was, then generate info for it
                    ...
                    let embed = Embed().SetTitle(com.Name).SetDescription(com.Description).AddField("Aliases", com.Alias |> String.concat ", ", true).AddField("Usage", com.Name + " " + com.Usage)
                    // Output the embed
                    embed
                        |> Message.Generate
                        |> messageCreated.RespondAsync
